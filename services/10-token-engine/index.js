 ...
 * Returns the existing log record if found.
 */
  ...
  try {
    ...
    await consumer.run({
      eachMessage: async ({ topic, partition, message }) => {
        ...
        const { driver_id, action_type, source_value, event_id, iso: payloadIso, physics_score, confidence_score, is_vpp_event, isVppEvent, is_high_fidelity, isHighFidelity } = payload;
        ...
        let confidenceScorePersist = confidence_score !== undefined ? parseFloat(confidence_score) : null;
        let isHighFidelityPersist = (is_high_fidelity === true || isHighFidelity === true) || (physicsScorePersist > 0.95) || (confidenceScorePersist > 0.95);
        ...
        if (isBehavioral) {
          // Fixed-value rewards (points/tokens) for behavioral actions (Proof of Physics exempt)
