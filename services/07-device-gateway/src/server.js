...
const localSafetyCache = {
  ...
  site: {}      // [L7-v5.13.0] Site-specific locks from L1/L2
};

async function updateLocalSafetyCache() {
  try {
    // 1. Fetch Global Safety Lock explicitly (L1 sync)
    const globalLockVal = await redis.get('l1:safety:lock');
    localSafetyCache.global = (globalLockVal === 'true' || globalLockVal === '1');

    const newRegionalLocks = {};
    const newSiteLocks = {};

    // 2. Scan for all regional grid locks (L4 sync) and site-specific safety locks (L1/L2 sync)
    let cursor = '0';
    do {
      const [newCursor, keys] = await redis.scan(cursor, 'MATCH', 'l*:*lock:*', 'COUNT', 100);
      cursor = newCursor;
      if (keys.length > 0) {
...
async function sendSetChargingProfile(chargePointId, limitKw, mode) {
    ...
    // [L7-v5.13.0] Enforce site-specific safety locks (L1/L2 site_safety sync)
